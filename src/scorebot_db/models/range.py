...
from scorebot.util import new, ScorebotError, get, lookup_address
...
class Range(Model):
    ...
    def get_flags(self, stolen=False):
        flags = list()
        for host in self.hosts.filter(enabled=True):
            flags += host.flags.filter(enabled=True, stolen__inull=(not stolen))
        return flags
...
class Beacon(Model):
    ...
    def update(self):
        ...
        if self.end is None and self.scored is None:
            ...
            reverse.save()
            General.info('Scored a Beacon by "%s" on Host "%s" owned by "%s" for "%d" PTS!' % (
    ...
    def expire(self):
        General.info('Beacon by "%s" on Host "%s" has expired after "%d" seconds!' % (
